...
#
#   This is a barebones lightweight implementation of 3DES (DES-EDE3), using a 
#   196-bit key and CBC with PKCS block padding. Python has been used to 
#   increase portability to other programs and languages. This implementation
#   has only been tested in Python and Ruby environments. Please use at own 
#   risk. This version is for Python 2.7x  systems. Python 3.x has different
...
#       k = triple_des(key, "\0\0\0\0\0\0\0\0")
#       d = k.encrypt(data)
...
class _baseDes(object):
    ...
    def _guardAgainstUnicode(self, data):
        ...
        if isinstance(data, unicode):
            raise ValueError("Cannot handle unicode data. Exiting.")
...
class triple_des(_baseDes):
    ...
    This algorithm uses the DES-EDE3 encryption methods via CBC (Cypher Block Chaining) modes.

    tri_des.des(key, [IV])

    key  -> Bytes containing the encryption key, must be 24 bytes long
    IV   -> Initial Value bytes, must be 8 bytes in length.
    """
    ...
    def setKey(self, key):
        # Will set the 24 byte crypto key.
        self.key_size = 24  # Use DES-EDE3 mode
        if len(key) != self.key_size:
            raise ValueError("Invalid key size. Key must 24 bytes long")
        if not self.getIV():
